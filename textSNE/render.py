...
wanted_set = pickle.load(open('../temp/wordset_basickeyword_LJ40K_FeelingWheel.pkl'))
##############################################sven tmp
...
def filter_points(points, wanted_wordset):
    new_points = [(title, x, y) for (title, x, y) in points if title in wanted_wordset]
    return new_points
